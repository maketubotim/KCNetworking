...
- (KCRequest *)requestWithSuccess:(KCSuccessHandler)success failure:(KCFailureHandler)failure {
    ...
    self.requestCachePolicy = KCRequestReturnCacheDataElseReloadRemoteData;
    
    //HTTPS就是一行代码这么简单
    [self appendAccessory:[KCSecurityPolicyAccessory defaultAccessory]];
...
- (NSString *)responseMessage {
    ...
    //KCRequest有默认文案,亦可获取self.response.responseJSONObject 自行过滤
    //修改默认文案请参阅 KCRequest.string
...
// 仿照 KCSecurityPolicyAccessory的设计 以及用法
